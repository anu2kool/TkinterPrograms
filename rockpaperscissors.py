...
if __name__ == '__main__':
    ...
    reset_frame=tk.Frame(master=root)
    reset_frame.grid(row=5, column=1)
    reset_button=tk.Button(master=reset_frame, text="reset", command=reset, relief=tk.RAISED, width=7, height=1, font=('Arial', 20))
    reset_button.grid(row=0, column=0, padx=150)
    
    exit_frame=tk.Frame(master=root)
    exit_frame.grid(row=0, column=2)
    exit_button=tk.Button(master=exit_frame, text="Exit", command=Exit, font=('Arial', 18), relief=tk.RAISED)
    exit_button.grid(row=0, column=0)
    
    max_wins=tk.Frame(master=root, highlightthickness=3, highlightbackground="blue")
    max_wins.grid(row=4, column=1)
    m_wins=tk.Label(master=max_wins, text="Maximum wins", font=('Arial', 19))
    m_wins.grid(row=0, column=0, padx=(0,10))
    m_player=tk.Label(master=max_wins, text="Player Name", font=('Arial', 19))
    m_player.grid(row=0, column=1)
    high_wins=tk.Label(master=max_wins, text="nil", font=('Arial', 16))
    high_wins.grid(row=1, column=0)
    high_name=tk.Label(master=max_wins, text="nil", font=('Arial', 16))
